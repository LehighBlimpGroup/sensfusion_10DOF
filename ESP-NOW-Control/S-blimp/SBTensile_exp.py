...
file = open('ESP-NOW-Control\S-blimp\Experiments\exp2' + str(anotherNewDate)+"_"+ str(yaw_cali)+ "_"+".csv","w")
file.write("time"+ " "+ "x"+ " "+"y" +" " +"z"+" " + "pitch"+" " +"roll"+" " + "yaw"+ " " + "waypoint_x" + " " + "waypoint_y" + " " + "waypoint_z" + " " + "yaw_rate" + " " +"\n")
...
if __name__ == "__main__":
    ...
    lead_z_pid = PID(.5, 0.01, .6, setpoint = 1.5)
    ...
    try:
        while True:
            ...
            if lead_robot_id in positions: # read the body frame indicated
                ...
                #     print(time_elapsed)
                period = 100
                cx, cy = find_point_on_circle(.4,.4, time_elapsed, period)
                ...
                xy_d = np.array([-0.63735521, -0.25039947])  + np.array([cx, cy])                    
                ...
                setpoint = np.array([xy_d[0], xy_d[1], absz, 0])

                if count >=10:
                    state = str(time.time() - time_start)+ " "+ str(positions[lead_robot_id][0])+ " "+str(positions[lead_robot_id][1]) +" " +str(positions[lead_robot_id][2])+" " + str(rotations[lead_robot_id][0])+" " + str(rotations[lead_robot_id][1])+" " + str(rotations[lead_robot_id][2])+ " "+ str(setpoint[0]) + " " + str(setpoint[1]) + " " + str(setpoint[2]) + " " + "\n"
                    print(state)
                    file.write(state)
                    count = 0
                    #pd.concat([df,data])
